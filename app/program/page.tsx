    ...
    <div className="min-h-screen bg-slate-950 pt-20">
      <div className="container mx-auto px-4 py-20">
        ...
        <Card className="bg-slate-900/30 border-slate-800 backdrop-blur-sm mb-20">
          <CardHeader>
            <CardTitle className="text-white text-2xl">Event Types</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {data.eventTypes.map((eventType: any, index: number) => (
                <div key={index} className="flex items-center gap-3">
                  <div className={`w-4 h-2 bg-${eventType.color}-500 rounded`} />
                  <span className="text-slate-300 font-medium">{eventType.label}</span>
                </div>
              ))}
            </div>
          </CardContent>
